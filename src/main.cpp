...
#if defined(NDEBUG)
# error "BitmillionGold cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfaf79916fa5475b8af7d3f989ce91f415eaa0116f9e44344b6981881d511fd37");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitmillionGold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitmillionGold Signed Message:\n";
...
{
    // BitmillionGold: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitmillionGold: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitmillionGold: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1519513526, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "rib das neves mg BR by leonardo rocha 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 70 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f4b8eb9d76106e9fac0683dbce406f439a85aaa1f6f565f88e6f875ab48d9f91d9cb5067a5cfd83bad517c2ea769733eb8f3af77502b8d04d8487c417b941eed") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519513526;
        ...
        assert(block.hashMerkleRoot == uint256("0xcf557a34618dd545e377a4fc933b696dbeee763a211969f415d231e5c1d3ffa2"));
...
unsigned char pchMessageStart[4] = { 0x52, 0x40, 0x73, 0x2d }; // BitmillionGold: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitmillionGoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitmillionGoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitmillionGoldMiner(CWallet *pwallet)
{
    printf("BitmillionGoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitmilliongold-miner");
    ...
    try { loop {
        ...
        printf("Running BitmillionGoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitmillionGoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitmillionGoldMiner, pwallet));
}
